...
class ClawAbility extends Ability {
    constructor() {
        super('Claw', 2, 4, 'FIRE', 'Deals damage to one opponent for a very small mana price.', 'single_enemy');
    }
...
class RoarAbility extends Ability {
    constructor() {
        super('Roar', 3, 0, 'NORMAL', 'Scares opponent making them deal less damage the entire game by 10%.', 'single_enemy');
    }
    ...
    execute(user, target) {
        ...
        if (target) {
            ...
            target.attack = Math.max(1, Math.round(target.attack * reduction));
...
class TigerBiteAbility extends Ability {
    constructor() {
        super('TigerBite', 9, 6, 'NORMAL', 'Deals damage and regenerates 30% of damage dealt.', 'single_enemy', 2);
    }
